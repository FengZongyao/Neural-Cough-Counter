import json
import torch
from pl_interface import Distillation_Interface
...
if __name__ == '__main__':
    ...
    with open('config.json', 'r') as config_file:
        config = json.load(config_file)["KD_Params"]

    check_point = ModelCheckpoint(dirpath = config['CHECKPOINT_DIR'],filename='KD'+'-best-{epoch:02d}-{valid_loss_frame:.4f}',
    ...
    model = Distillation_Interface(lr = config['LR'],temperature = config['TEMPERATURE'],mu=config['MU'])
    
    logger = TensorBoardLogger(config['LOG_DIR'], name='KD')
    ...
    model.load_teacher(config['TEACHER_CHECKPOINT'])
